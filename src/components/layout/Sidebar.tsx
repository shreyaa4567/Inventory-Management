...
const billingNavItems = [
  { to: '/supplier-bills', label: 'Supplier Bills', icon: FileInput },
  { to: '/customer-bills', label: 'Customer Bills', icon: FileOutput },
];
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const renderNavItem = (item: { to: string; label: string; icon: React.FC<{ size?: number; className?: string }> }) => (
    ...
    <>
      ...
      <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <nav className="sidebar-nav">
          ...
          <div className="sidebar-nav-label" style={{ marginTop: '1rem' }}>Billing</div>
          {billingNavItems.map(renderNavItem)}
          <div className="sidebar-nav-label" style={{ marginTop: '1rem' }}>Other</div>
