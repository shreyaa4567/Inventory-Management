...
const SupplierBillsPage: React.FC = () => {
  const { suppliers, products, supplierBills, addSupplierBill } = useInventory();
  ...
  const handleSave = () => {
    ...
    if (!supplier || lineItems.some(it => !it.productId || it.quantity <= 0)) return;
    addSupplierBill(
      supplier.id,
      supplier.name,
  ...
  return (
    <div>
      ...
      <div className="fade-in-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
        <div>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 700 }}>Supplier Bills (Import Stock)</h3>
          <p style={{ fontSize: '0.85rem', color: '#6b7280', marginTop: 2 }}>Generate invoices when products arrive from suppliers</p>
        </div>
        <button className="btn btn-primary" onClick={() => setShowForm(true)}>
          <Plus size={18} /> New Supplier Bill
        </button>
      </div>

      {/* Create Bill Form */}
      {showForm && (
        <div className="content-card fade-in-up" style={{ marginBottom: '1.5rem', borderLeft: '4px solid #3b82f6' }}>
          <div className="content-card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3><FileText size={18} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 8, color: '#3b82f6' }} />New Supplier Bill</h3>
          ...
          <div style={{ padding: '1rem 1.5rem' }}>
            ...
            <div className="form-group" style={{ maxWidth: 400, marginBottom: '1.25rem' }}>
              ...
              <select
                ...
                style={{ width: '100%', padding: '0.75rem 1rem', border: '1.5px solid #e5e7eb', borderRadius: 12, fontSize: '0.95rem', fontFamily: 'var(--font-sans)', background: '#fafafa' }}
              >
                  ...
                  <tr key={idx}>
                    ...
                    <td>
                      <input
                        ...
                        style={{ width: 80, padding: '0.5rem', border: '1.5px solid #e5e7eb', borderRadius: 8, fontSize: '0.88rem', textAlign: 'center' }}
                      />
                    </td>
                    <td>
                      <input
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                <div style={{ fontSize: '1.1rem', fontWeight: 700 }}>
                  Grand Total: <span style={{ color: '#2563eb' }}>₹{grandTotal.toLocaleString()}</span>
                </div>
                <button
                  className="btn btn-primary"
                  onClick={handleSave}
                  disabled={!selectedSupplier || lineItems.some(it => !it.productId || it.quantity <= 0)}
                >
      ...
      <div className="content-card fade-in-up" style={{ animationDelay: '0.1s' }}>
        <div className="content-card-header">
          <h3>Bill History ({supplierBills.length})</h3>
        </div>
        <table className="data-table">
          ...
          <tbody>
            {supplierBills.length === 0 ? (
              <tr><td colSpan={6} style={{ textAlign: 'center', padding: '2rem', color: '#9ca3af' }}>No supplier bills yet. Click "New Supplier Bill" to create one.</td></tr>
            ) : (
              supplierBills.map(bill => (
                <tr key={bill.id}>
                  <td style={{ fontWeight: 700, color: '#2563eb' }}>{bill.id}</td>
                  <td style={{ fontWeight: 600 }}>{bill.supplierName}</td>
                  <td>
